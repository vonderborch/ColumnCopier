...
namespace ColumnCopier.prev
{
    ...
    public class OldRequest
    {
        ...
        public OldRequest(string rawText, int requestNumber, bool hasColumnHeaders, bool cleanData)
        {
            ID = requestNumber;
        ...
        public OldRequest(XElement xmlData)
        {
            foreach (var node in xmlData.Elements())
            {
                ...
                {
                    ID = OldMain.ParseTextToInt(node.Value);
                }
                else if (nodeName == "PreserveRequest")
                {
                    PreserveRequest = OldMain.ParseTextToBool(node.Value);
                }
                else if (nodeName == "ColumnKeys")
                {
                    foreach (var column in node.Elements())
                    {
                        ...
                        foreach (var columnItem in column.Elements())
                        {
                            ...
                            {
                                case "Key":
                                    key = OldMain.ParseTextToInt(columnItem.Value);
                                    break;
                                case "Value":
